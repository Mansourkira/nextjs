...
const Result = () => {
  ...
  useEffect(() => {
    ...
    setRes(Sum);
  }, [Stats.UserArray]); // Include Stats.UserArray in the dependency array

  const generatePDF = () => {
    const doc = new jsPDF();
    API.forEach((Elem, Ind) => {
      doc.text(`Question # ${Ind + 1}: ${Elem.Question}`, 10, 10 + Ind * 10);
      doc.text(
        `Your Answer: ${Elem.Options[Stats.UserArray[Ind]]}`,
        10,
        15 + Ind * 10
      );
    });
        ...
        <div className="Left sm:col-span-7 sm:row-span-2">
          <div className="A bg-[#34495e] mb-2 p-3 space-x-5 justify-between All items-center ">
            ...
            <div className="P-1 bg-black rounded-full h-[100px] w-[100px] flex justify-center items-center overflow-hidden">
              <p
                style={{
                  background: `conic-gradient(#4bcffa 0 ${
                    (Res / API.length) * 100
                  }%, transparent 0 0%)`,
                }}
